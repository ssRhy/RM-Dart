 ...
 *               第1层：chassis（类比摩擦轮）—— 底盘到位是后续动作的前提
 *               第2层：feed（类比拨弹盘首步）—— chassis 到位后执行供弹
 *               第3层：trans（类比拨弹盘末步）—— feed 到位后执行横移
 *             CAN 帧分配：
...
void DartMainInit(void)
{
    ...
    /* ----- Trans（ID3，M3508） ----- */
    MotorInit(&DART.trans_motor, 3, 1, DJI_M3508, 1, 1.0f, 0);
...
/**
 * @brief          设置模式（时序约束核心）
 * @note           层级关系（参照 shoot 模块 state + mode 双门禁）：
...
void DartMainObserver(void)
{
    ...
    DART.chassis_fdb.speed_fdb = DART.chassis_motor.fdb.vel;
    ...
    DART.chassis_fdb.angle_fdb = (DART.chassis_ecd_count * ECD_RANGE + DART.chassis_motor.fdb.ecd) * MOTOR_ECD_TO_ANGLE;
...
/**
 * @brief          设置目标量（三层 switch，与 ShootReference 双层结构对应）
