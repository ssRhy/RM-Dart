...
#define cali_flash_read(address, buf, len) \
    flash_read((address), (buf), (len))  //flash read function, flash 读取函数
#define cali_flash_write(address, buf, len) \
    flash_write_single_address((address), (buf), (len))  //flash write function,flash 写入函数
#define cali_flash_erase(address, page_num) \
    flash_erase_address((address), (page_num))  //flash erase function,flash擦除函数

#define get_remote_ctrl_point_cali() \
    get_remote_control_point()  //get the remote control point，获取遥控器指针
#define gyro_cali_disable_control() \
    RC_unable()  //when imu is calibrating, disable the remote control.当imu在校准时候,失能遥控器
...
#define RC_CALI_VALUE_HOLE \
    600  //remote control threshold, the max value of remote control channel is 660.
