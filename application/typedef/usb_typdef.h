...
typedef struct
{
    ...
    struct
    {
        ...
        uint8_t type;
        float data;
    } __packed packages[DEBUG_PACKAGE_NUM];

    uint16_t checksum;
} __packed SendDataDebug_s;
...
typedef struct
{
    ...
    struct
    {
        struct
        {
            ...
            float vy;
            float wz;
        } __packed speed_vector;

    } __packed data;

    uint16_t crc;
} __packed SendDataRobotMotion_s;
...
typedef struct RobotCmdData
{
    ...
    uint16_t checksum;
} __packed ReceiveDataRobotCmd_s;
...
typedef struct
{
    ...
    uint16_t crc;
} __packed ReceiveDataPidDebug_s;
#endif  // USB_TYPEDEF_H
